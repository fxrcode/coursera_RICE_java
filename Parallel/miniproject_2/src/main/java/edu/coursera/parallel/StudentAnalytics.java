...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        // stream all students no longer active, and merge to a name->count map.
        nameCounts = Stream.of(studentArray)
                .parallel()
                .filter(s -> !s.checkIsCurrent())
                .map(s -> s.getFirstName())
                .collect(
                    Collectors.toMap(   // in case of key collision, need to supply keyMapper, valueMapper, mergeFunction
                        Function.identity(),
                        name->1,
